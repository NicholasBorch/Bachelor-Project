...
# The only modification relative to the original implementation is the
# image transform used during noise generation. The original used:
#
#     transforms.Compose([
...
#     ])
#
# This version adds ImageNet channel-wise standardisation:
#
#     transforms.Compose([
#         transforms.Resize((image_size, image_size)),
#         transforms.ToTensor(),                        # → [0, 1]
#         transforms.Normalize(                         # → ~N(0, 1) per channel
#             mean=[0.485, 0.456, 0.406],
#             std=[0.229, 0.224, 0.225],
#         ),
#     ])
#
# Effect: pixel values are centred around zero (~half positive, half negative).
# This introduces genuine cancellation in the dot product x @ W_y, making
# the score ordering sensitive to actual image content rather than being
# dominated entirely by the sign structure of the frozen W matrix.
# Expected effect: lower flip concentration, more instance-dependent noise.
#
# Everything else — W construction, flip rate sampling, masking, softmax,
# transition row construction, multinomial sampling — is identical.
# ─────────────────────────────────────────────────────────────────────────────
...
def generate_instance_dependent_noisy_labels(
    ...
    *,
    ...
    # ── CHANGED: added ImageNet Normalize after ToTensor ─────────────────────
    # Original: transforms.Compose([Resize, ToTensor])
    # Now:      transforms.Compose([Resize, ToTensor, Normalize])
    #
    # Before: x_i ∈ [0, 1]        — all non-negative, no cancellation in x @ W_y
    # After:  x_i ≈ N(0, 1)       — centred, ~half negative, genuine cancellation
    # ─────────────────────────────────────────────────────────────────────────
    tfm = transforms.Compose([
        transforms.Resize((image_size, image_size)),
        transforms.ToTensor(),
        transforms.Normalize(                           # ← THIS IS THE ONLY CHANGE
            mean=[0.485, 0.456, 0.406],
            std=[0.229, 0.224, 0.225],
        ),
    ])
    ...
    for x, y, _ in tqdm(dl, desc="IDN corruption (standardised)", leave=True):
        b = x.size(0)
        x = x.to(device)        # (B, 3, H, W) — standardised to ~N(0,1) per channel
        y = y.to(device).long()

        # Flatten images to pixel vectors and project through W_y (Xia et al. Eq. 1)
        # x_flat ∈ R^(B × d), values now centred around zero
        x_flat = x.view(b, -1)
        W_y = W[y]
