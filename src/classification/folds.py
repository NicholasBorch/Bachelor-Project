...
def make_inner_folds_lesion_stratified(df: pd.DataFrame, n_splits: int, seed: int) -> pd.DataFrame:
    # Stratified inner folds on unique lesion_id within an outer training split
    # Mirrors the outer fold construction exactly, operating on the outer train subset
    df = df.copy().sort_values(["lesion_id", "image_id"]).reset_index(drop=True)
    lesion_df = df.drop_duplicates(subset=["lesion_id"]).sort_values("lesion_id").reset_index(drop=True)
    ...
    df["inner_fold"] = df["lesion_id"].astype(str).map(lesion_to_fold)
