"""Post-search analysis of an Optuna study.

Reads the SQLite study written by scripts/optuna_search.py and produces:

  1. summary.txt        — top-K trials, best params, runtime stats
  2. all_trials.csv     — every trial's params + objective for spreadsheet inspection
  3. best_config.yaml   — drop-in config for configs/method/<method>_tuned.yaml
  4. plots/
       optimization_history.png   — objective over trial number
       param_importances.png      — fANOVA-derived parameter importance
       parallel_coordinate.png    — every trial as a polyline across params
       slice.png                  — one-parameter projections of the objective
       contour_pairs.png          — top-2 parameter interaction contours
       trial_validation_curves.png — per-trial smoothed val BA curves
                                     (top-K colored, others grey)

The validation-curve plot is custom; the rest come from optuna.visualization
(matplotlib backend) which is bundled with optuna.

Usage:
    python -m scripts.optuna_analyze \
        --method asyco_divmix --fold 5 \
        --tau 0.2 --top-k 5
"""
...
def _trial_smoothed_curve(log_path: Path, smooth_window: int = 5) -> np.ndarray | None:
    """Return the smoothed val_balanced_accuracy curve for one trial, or None."""
...
def _summary_table(study: optuna.Study, top_k: int) -> str:
    ...
    lines.append(
        "  " + "  ".join(f"{c:>10s}" for c in (
            "trial", "value", *sorted_completed[0].params.keys()
        ))
    )
    for t in sorted_completed[:top_k]:
        cells = [f"{t.number:>10d}", f"{t.value:>10.4f}"]
        for k in sorted_completed[0].params.keys():
            v = t.params.get(k)
            cells.append(f"{v:>10.4g}" if isinstance(v, (int, float)) else f"{v!s:>10s}")
...
def _save_optuna_visualizations(study: optuna.Study, plot_dir: Path) -> None:
    """Use optuna.visualization.matplotlib for save-friendly plots."""
    ...
    completed = [t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE]
    if len(completed) < 2:
        print(f"[analyze] Only {len(completed)} completed trials; skipping built-in plots.")
        return
    ...
    # Importance and contour need ≥2 completed trials AND
    # the right structural conditions; guard each.
    if len(completed) >= 5:
        plots.append(("param_importances", lambda: ovm.plot_param_importances(study)))

    for name, fn in plots:
        try:
            ax = fn()
            # ovm returns a matplotlib Axes; grab its figure.
    ...
    # Contour plot — pick the top 2 most important parameters if >= 2 params
    if len(completed) >= 5:
        try:
            ...
            if len(top_two) == 2:
                ...
                plt.close(fig)
                print(f"[analyze] saved {plot_dir / 'contour_pairs.png'} for {top_two}")
        except Exception as e:
...
def _plot_validation_curves(
    ...
    """Custom plot: per-trial smoothed val BA curves, with top-K trials
    highlighted by color and remaining trials drawn in light grey."""
    completed = [t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE]
    if not completed:
        return
    ...
    # Light-grey background curves for non-top trials
    for t in completed:
        if t.number in top_trial_numbers:
            continue
        log_path = out_dir / "per_trial" / f"trial_{t.number:04d}" / "training_log.jsonl"
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("smoothed validation balanced accuracy")
...
def _write_best_config_yaml(method: str, study: optuna.Study, path: Path) -> None:
    """Drop-in tuned config: same keys as configs/method/<method>.yaml but
    with the best Optuna hyperparams substituted in."""
    completed = [t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE]
    if not completed:
        return
    best = max(completed, key=lambda t: t.value)

    # Read the original method config so we keep keys not in the search space.
    root = project_root()
    ...
    with path.open("w") as f:
        f.write(
            "# Hyperparameters chosen by Optuna search.\n"
...
def main(args: argparse.Namespace) -> int:
    ...
    if not out_dir.exists():
        print(f"ERROR: {out_dir} does not exist. Run optuna_search.py first.",
              file=sys.stderr)
        return 1
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Post-search Optuna analysis: summary + plots + best-config dump",
    )
    ...
    p.add_argument("--top-k", default=5, type=int,
                   help="Highlight top-K trials in summary and curve plot.")
